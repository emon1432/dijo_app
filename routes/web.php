...
Route::get('/test-config', function () {
    return response()->json([
        'value' => config('get_payment_publish_status')
    ]);
});
...
if (!$is_published) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'payfast', 'as' => 'payfast.'], function () {
            Route::get('pay', [PayFastPaymentController::class, 'index'])->name('pay');
            Route::get('return', [PayFastPaymentController::class, 'return'])
                ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
            Route::post('notify', [PayFastPaymentController::class, 'notify'])
                ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
            Route::get('cancel', [PayFastPaymentController::class, 'cancel'])
                ->withoutMiddleware([\App\Http\Middleware\VerifyCsrfToken::class]);
        });
