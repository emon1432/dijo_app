    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','pos_registration_mail_status_user')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Receive_Mail_On_New_Customer_Registration_on_POS')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_user_added_through_POS,_they_will_receive_an_automated_confirmation_mail_with_their_email_&_password.')}}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-title-on="{{translate('Want_to_enable_User_POS_Registration_mail?')}}"
                                data-title-off="{{translate('Want_to_disable_User_POS_Registration_mail?')}}"
                                data-text-on="<p>{{translate('If_enabled,_customers_will_receive_a_confirmation_email_that_their_registration_was_successfull_and_get_the_email_and_password_in_the_mail.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['user','pos-registration',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','user')->where('email_type', 'pos_registration')->first())
                                    ...
                                    <div class="__bg-F8F9FC-card default-form lang_form" id="default-form">
                                        ...
                                        <div class="form-group mb-0">
                                            <label class="form-label">
                                                {{ translate('Mail_2nd_Body_Message') }}({{ translate('messages.default') }})
                                                <span class="input-label-secondary text--title" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Write_the_mail_body_message_within_75_words')}}">
                                                    <i class="tio-info-outined"></i>
                                                </span>
                                            </label>
                                            <textarea class="form-control" id="ckeditor2" data-id="mail-body2" name="body_2[]">
                                                {!! $data?->getRawOriginal('body_2') !!}
                                            </textarea>
                                        </div>
                                    </div>
                                    @foreach(json_decode($language) as $lang)
                                    ...
                                    if($data && count($data['translations'])){
                                        ...
                                        foreach($data['translations'] as $t)
                                        {
                                            ...
                                            if($t->locale == $lang && $t->key=="body_2"){
                                                $translate[$lang]['body_2'] = $t->value;
                                            }
                                        }
                                    }
                                        ?>
                                        <div class="__bg-F8F9FC-card d-none lang_form" id="{{$lang}}-form1">

                                            <div class="form-group mb-0">
                                               <label class="form-label">
                                                    {{ translate('Mail_2nd_Body_Message') }}({{strtoupper($lang)}})
                                                    <span class="input-label-secondary text--title" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Write_the_mail_body_message_within_75_words')}}">
                                                        <i class="tio-info-outined"></i>
                                                    </span>
                                                </label>
                                                <textarea class="ckeditor form-control" name="body_2[]">
                                                   {!! $translate[$lang]['body_2']??'' !!}
                                                </textarea>
                                            </div>
                                        </div>
                                    @endforeach
                                @else
                                    <div class="__bg-F8F9FC-card default-form">
                                        ...
                                        <div class="form-group mb-0">
                                              <label class="form-label">
                                                {{ translate('Mail_2nd_Body_Message') }}
                                                 <span class="input-label-secondary text--title" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Write_the_mail_body_message_within_75_words')}}">
                                                        <i class="tio-info-outined"></i>
                                                    </span>
                                            </label>
...
@push('script_2')
    ...
    let editor2 = CKEDITOR.replace('ckeditor2');
    editor2.on( 'change', function( evt ) {
        $('#mail-body2').empty().html(evt.editor.getData());
    });
    </script>
    <!-- Email Template End-->
@endpush
