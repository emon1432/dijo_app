    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','new_advertisement_mail_status_admin')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Receive_Mail_on_‘New_Advertisement_Request’')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_store_adds_an_adversement,_the_admin_will_receive_an_automated_email_on_New_Advertisement_Request.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_New_Advertisement_Request?')}}"
                                       data-title-off="{{translate('Want_to_disable_New_Advertisement_Request?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_the_admin_will_receive_an_automated_email_on_New_Advertisement_Request')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_the_admin_won\'t_receive_an_automated_email_on_New_Advertisement_Request.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['admin','new-advertisement',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'new_advertisement')->first())
                 @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['admin','new-advertisement']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','admin')->where('email_type', 'new_advertisement')->first())
