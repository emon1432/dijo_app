    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','update_advertisement_mail_status_admin')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Receive_Mail_on_‘Advertisement_update_Request’')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_store_adds_an_adversement,_the_admin_will_receive_an_automated_email_on_Advertisement_update_Request.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_Advertisement_update_Request?')}}"
                                       data-title-off="{{translate('Want_to_disable_Advertisement_update_Request?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_the_admin_will_receive_an_automated_email_on_Advertisement_update_Request')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_the_admin_won\'t_receive_an_automated_email_on_Advertisement_update_Request.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['admin','update-advertisement',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'update_advertisement')->first())
                 @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['admin','update-advertisement']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','admin')->where('email_type', 'update_advertisement')->first())
                                        ...
                                        <div class="__bg-F8F9FC-card">
                                            <div class="row g-3">
                                                <div class="col-sm-6">
                                                    @if ($language)
                                                        <div class="form-group m-0 lang_form default-form">
                                                            ...
                                                            <input type="text" maxlength="15" data-id="mail-button" name="button_name[]"  placeholder="{{translate('Eg:_Order_now')}}" class="form-control h--45px" value="{{ $data?->getRawOriginal('button_name') }}">
                                                        </div>
                                                    @foreach(json_decode($language) as $lang)
                                                    <?php
                                                    if($data && count($data['translations'])){
                                                        ...
                                                        <div class="form-group m-0 d-none lang_form" id="{{$lang}}-form1">
                                                            ...
                                                            <input type="text" maxlength="15" name="button_name[]"  placeholder="{{translate('Eg:_Order_now')}}" class="form-control h--45px" value="{{ $translate[$lang]['button_name']??'' }}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                <div class="form-group m-0">
                                                    ...
                                                    <input type="text" maxlength="15" placeholder="{{translate('Eg:_Order_now')}}" class="form-control h--45px" name="button_name[]" value="">
                                                </div>
                                                @endif
                                                </div>
                                                <div class="col-sm-6">
                                        ...
                                        <div class="__bg-F8F9FC-card">
                                            ...
                                            <div class="form-group mb-0">
                                                @if ($language)
                                                       <div class="form-group lang_form default-form">
                                                            ...
                                                            <input type="text" maxlength="50" data-id="mail-copyright" name="copyright_text[]"  placeholder="{{ translate('Eg:_Copyright_2023_Stackfood._All_right_reserved')}}" class="form-control" value="{{ $data?->getRawOriginal('copyright_text') }}">
                                                        </div>
                                                    @foreach(json_decode($language) as $lang)
                                                    <?php
                                           $translate = [];
                                           if($data && count($data['translations'])){
                                                        ...
                                                        <div class="form-group d-none lang_form" id="{{$lang}}-form3">
                                                            ...
                                                            <input type="text" maxlength="50" name="copyright_text[]"  placeholder="{{ translate('Eg:_Copyright_2023_Stackfood._All_right_reserved')}}" class="form-control" value="{{ $translate[$lang]['copyright_text']??'' }}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" maxlength="50"  placeholder="{{ translate('Eg:_Copyright_2023_Stackfood._All_right_reserved')}}"class="form-control" name="copyright_text[]" value="">
                                                </div>
                                                @endif
                                            </div>
                                        </div>
