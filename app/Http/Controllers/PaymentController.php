...
class PaymentController extends Controller
{
    ...
    public function payment(Request $request)
    {
        ...
        $content = json_encode($request, JSON_PRETTY_PRINT);
    
        // Or, if you prefer a simpler format:
        // $content = print_r($data, true);
    
        // Save it to a .txt file (e.g. storage/app/payment_logs/)
        $filename = 'payment_json_' . now()->format('Ymd_His') . '.txt';
        Storage::put('payment_logs/' . $filename, $request['customer_id'].'---'.$request['payment_platform'].'---'.$request->order_id);
